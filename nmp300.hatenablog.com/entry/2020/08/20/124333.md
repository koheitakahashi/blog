...
## はじめに
少し前に以下の記事で、自分が担当したActionCableを用いた機能実装について解説しました。
<!-- 前回のブログのリンク -->

上記の記事を書いているうちに、機能実装を担当することになった当初の気持ちが蘇ってきました。
...
そこで、この記事ではそういう「ActionCableナニモワカラナイ」という方々に向けて自分なりの概要を書きました。  
...
HTTPプロトコルで、リアルタイムで双方向通信を行うためには一度確立したHTTPコネクションを開いたままにして、定期的にリクエストを送るという方法などをする必要がありました。

しかし、その方法は本来想定されているHTTPプロトコルの使い方とは外れた、いわゆる裏技的なものでした。
...
そして、以下のようなレスポンスを返します。  
もし、ステータスコードが101以外なら、WebSocketに移行しません。
```
...
このように、consumerがchannelをsubscribeして、そのsubscriberにデータがbroadcastされることで、データがクライアントに送られるという流れになります。
## 具体的にどのように書くのか
上記に、基本的なデータのやりとりの流れを説明しました。  
ここからは、以下の記事で解説した私のコードを元に、上記のことをどのようにコードで書いていけば良いのかを説明します。

<!-- 前回のブログのリンク -->

### ハンドシェイクを送る
まず、フロントエンド側でconsumerを設定する必要があります。

`rails new`した後のプロジェクトには、`app/javascript/channels/consumer.js`に以下のように記述されていると思います。  
consumerを特に設定する必要がない場合はここはいじらなくても問題ないと思います。
```javascript
...
`app/channels/application_cable/connection.rb`に処理を記述することで、ハンドシェイクが送られた後、どのような時にWebSocketコネクションを確立するのか、しないのかを設定することができます。

以下のRailsガイドの例だと、cookieに保存されているuser_idが、DBに保存されている場合は、WebSocketコネクションを確立して、それ以外の時は確立しないようにしているのが分かります。
...
```
[Railsガイド ActionCableの章](https://railsguides.jp/action_cable_overview.html#%E3%82%B5%E3%83%BC%E3%83%90%E3%83%BC%E5%81%B4%E3%81%AE%E3%82%B3%E3%83%B3%E3%83%9D%E3%83%BC%E3%83%8D%E3%83%B3%E3%83%88-%E3%82%B3%E3%83%8D%E3%82%AF%E3%82%B7%E3%83%A7%E3%83%B3)

### subscribeしたときの処理
`connection.rb`により、WebSocketコネクションを確立するか否かの判断がされ、無事コネクションが確立された後、上記で指定したchannelをsubscribeすることとなります。

ここで、channelの処理を担う`app/channels/timelines_channel.rb`が存在と仮定して説明します。

以下のように、`subscribe`メソッドを定義すると、TimelinesChannelがsubscribeされたときの処理を記述することができます。

例えば、以下では、TimelinesChannelがsubscribeされると、過去の分報を10件broadcastすることになります。  
...
```
### broadcastする
上記でも触れましたが、以下のように記述することで任意のchannelのsubscriberにデータをbroadcastすることができます。
...
3. subscriptionが確立されようとしたことでハンドシェイクが送られる
...
このような設定や処理を記述して、ActionCableがWebSocket通信を実現し、データがやりとりされるのだということが少しでも伝わったなら幸いです。

自分は上記のように理解して、この記事を書きましたが、理解や記述が間違っているところなどがあれば教えていただけると大変嬉しいです。
## 参考にさせていただいた資料
