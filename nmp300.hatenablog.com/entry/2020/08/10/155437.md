...
スクラム開発のプラクティスについて詳しく知りたい方は、以前に私が書いた拙文がありますので、そちらをご覧ください。
...
そして、以下が私が作成したPRです。現在は他の受講生の方が実装したコードもマージされている状態です。  
この記事を書いている段階では、必要な機能が未実装なので本番にリリースされてはいません(デザインもまだです)。  

また、本記事で以下に解説するコードの例は私がPRを出してレビューが通った時点のコードになりますので、ご了承ください。

https://github.com/fjordllc/bootcamp/pull/1527

ActionCableとVue.jsを使用して、上記のリアルタイムでCRUDができる分報機能を実装しました(大きな機能だったため、私は最小限の機能の実装し、大枠を作って、その他必要な機能の追加は他の受講生の方に任せたという運びです)。  

正直なところ、このissueに着手し始めた当初は、「Vue.jsナニモワカラン」、「ActionCableナニソレワカラン」という感じで四苦八苦しておりましました。

日本語・英語で検索しても、ActionCableの具体的な実践例が載っている記事をあまり見なかったため、これからActionCableを使われる方に少しでも参考になればと思い、この記事を書きました。  
...
- 受講生が毎日の学習の記録を綴った日報を作成することができる。
- 提出物や日報に対してメンターの方や受講生の方がコメントをつけられる…などなど。
...
#### Vue.js側
Vue.js側はchannelへのsubscribeや、channel内のデータを取得して処理するということを管理するための、`timelines-channel.vue`というコンポーネントと、その子コンポーネントで、一つ一つの分報の描画などを管理する`timeline.vue`というコンポーネントを作成しました。
...
しかし、一つ一つ頭を悩ませながら理解して、そしてできる方々に教えていただきながら少しずつ実装を進めていき、なんとかPRを出すところまで進めることができました。  
